...
class OTEstimators {
 public:
  ...
  float flowtree_query(const std::vector<std::pair<int32_t, float>> &a,
    ...
    float quadtree_query(const std::vector<std::pair<int32_t, float>> &query,
                         const std::vector<std::pair<int32_t, float>> &point){
        auto query_embedding = compute_embedding(query);
        auto point_embedding = compute_embedding(point);
        float score = 0;
        size_t qp = 0;
        size_t dp = 0;
        while (qp < query_embedding.size() || dp < point_embedding.size()) {
...
PYBIND11_MODULE(ot_estimators, m) {
  ...
  py::class_<OTEstimators>(m, "OTEstimators")
      ...
      .def("flowtree_query", &OTEstimators::flowtree_query)
      .def("quadtree_query", &OTEstimators::quadtree_query);
}
